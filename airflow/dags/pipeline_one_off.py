...
with DAG(
    ...
    predict = BashOperator(
        task_id="predict",
        bash_command=_cmd("src.inference.run_batch_predict", '--run-ts "{{ dag_run.conf.get(\'run_ts\', \'\') }}"'),
    )

    evaluate = BashOperator(
        task_id="evaluate",
        bash_command=_cmd("src.eval.run_evaluator", '--run-ts "{{ dag_run.conf.get(\'run_ts\', \'\') }}"'),
