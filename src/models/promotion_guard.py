...
class PromotionGuardConfig:
    ...
    min_prediction_nunique: int = 2
    min_prediction_std: float = 1e-12
...
def _prediction_dispersion(rows: pd.DataFrame) -> tuple[int, float]:
    values = pd.to_numeric(rows["y_pred"], errors="coerce").to_numpy(dtype=float)
...
def evaluate_candidate_promotion_guard(
    *,
    ...
    try:
        ...
        prediction_nunique, prediction_std = _prediction_dispersion(rows)
        if prediction_nunique < cfg.min_prediction_nunique or not np.isfinite(prediction_std):
            return PromotionGuardResult(
                allowed=False,
                reason=(
                    f"candidate predictions are degenerate for model_name={matched_name or candidate_name} "
                    f"(nunique={prediction_nunique}, std={prediction_std})"
                ),
    ...
    # Only expert candidates are expected to carry feature envelope metadata.
    if str(candidate_ref.model_type).strip().lower() != "expert":
        return PromotionGuardResult(
            allowed=True,
            reason="candidate is not an expert model; prediction dispersion check passed",
            candidate_model_name=matched_name or candidate_name,
            prediction_nunique=prediction_nunique,
            prediction_std=prediction_std,
        )
